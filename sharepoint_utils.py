...
class SharePointNavigator:
    ...
    def get_site_id(self):
        ...
        # Parse the URL
        site_path = SITE_URL.replace("https://", "").replace("http://", "")
    ...
    def get_drive_id(self, site_id):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Cerca la library corretta
                # Prova match esatti e varianti comuni
                search_names = [
                    LIBRARY_NAME,
                    "Shared Documents",
                    "Documents", 
                ...
                # Se non trova, usa la prima (di solito è quella principale)
                if drives:
                    print(f"⚠️  Uso library default: {drives[0]['name']}")
    ...
    def navigate_to_folder(self, site_id, drive_id, folder_path):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print("⚠️  Path completo non trovato, navigazione step-by-step...\n")
    ...
    def _navigate_progressively(self, site_id, drive_id, folder_path):
        ...
        # Prima ottieni contenuto della root
        ...
        # Ora naviga cartella per cartella
        current_path = ""
        for i, folder_name in enumerate(folders):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Se siamo all'ultima cartella, abbiamo finito
                    if i == len(folders) - 1:
                        print(f"\n✅ Raggiunto: {successful_path}\n")
                        return current_items, successful_path
                    
                    # Altrimenti mostra sottocartelle disponibili
                    ...
                    if subfolders and i < len(folders) - 1:
                        ...
                        if next_folder not in subfolders:
                            print(f"     ⚠️  '{next_folder}' non trovata")
                            print(f"     Cartelle disponibili: {', '.join(subfolders[:5])}")
                            break
                else:
                    print(f"     ✗ Errore {response.status_code}")
                    
                    # Se fallisce, mostra cosa c'è nella cartella precedente
                    if successful_path:
                        print(f"\n⚠️  Fermato a: {successful_path}")
                        return current_items, successful_path
                    break
