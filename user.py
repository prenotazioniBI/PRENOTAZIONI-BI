...
def menu_utente(df_centralizzato, servizi_scelti, nav):
    """
    Gestisce l'inserimento delle richieste dell'utente
    
    Args:
        df_centralizzato: DataFrame centralizzato (usato solo per statistiche/grafici)
        servizi_scelti: Lista dei servizi selezionati
        nav: Oggetto SharePointNavigator
    
    Returns:
        df_aggiornato: DataFrame con le nuove richieste
        successo: Boolean
        messaggio: Stringa con il risultato
    """
    user = st.session_state.get("user")
    ...
    if df_personale is None:
        return df_centralizzato, False, "Errore nel caricamento delle richieste personali"
